...
import { ExisteProducto, ExisteProducto2 } from "../helpers/ComprobarProductoExiste.js"
...
export const RegistrarCompra = async (req = request, res = response) => {
    try {
        ...
        const resultado = await body.COD_PRODUCTO.map((elemento) => {
            return ExisteProducto2(elemento);
        })

        let falso = [] = await Promise.all(resultado);

        let falso2 = falso.filter((elemento) => elemento == false)


        if (falso2.length > 0) {
            return res.status(400).json({
                ok: false,
                msg: 'PRODUCT NOT FOUND2',
