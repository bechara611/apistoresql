...
body.COD_PRODUCTO.forEach(async (codigo, index) => {
    ...
    const existe =await ConexionSQL.query('SELECT * FROM PRODUCTOS WHERE COD_PRODUCTO=?',[codigo])
    if(existe.length==0){
         return res.status(400).json({
            ok: false,
            msg: error?.sqlMessage || 'INTERNAL ERROR'
    ...
    body:{id:compraRegistrada?.insertId,...body},
        ...
        body.COD_PRODUCTO.forEach(async (codigo, index) => {
            if (ventaRegistrada.insertId) {
                ...
                await ConexionSQL.query('INSERT INTO VENTAS_DETALLE(COD_PRODUCTO,CANTIDAD,PAGADO,ID_VENTA) VALUES (?,?,?,?)', [codigo, body.CANTIDAD[index], body.PAGADO[index], compraRegistrada?.insertId])
            }
        });
