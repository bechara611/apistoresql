...
export const RegistrarCompra = async (req = request, res = response) => {
    try {
        ...
        const elemento2 = body.COD_PRODUCTO.map((elemento)=>{return console.log(elemento)})
        console.log({elemento2})
       const elemento = await body.COD_PRODUCTO.forEach(async (element) => {
            const respu = await ExisteProducto(element)
            if (respu == false) {
